...
TEST_FILES_URL = "https://raw.githubusercontent.com/microsoft/markitdown_liberated/refs/heads/main/packages/markitdown_liberated/tests/test_files"
...
def test_guess_stream_info(test_vector):
    ...
    with open(local_path, "rb") as stream:
        guesses = markitdown_liberated._get_stream_info_guesses(
            stream,
...
def test_convert_stream_without_hints(test_vector):
    ...
    with open(os.path.join(TEST_FILES_DIR, test_vector.filename), "rb") as stream:
        result = markitdown_liberated.convert(stream, url=test_vector.url)
        for string in test_vector.must_include:
            assert string in result.markdown
        for string in test_vector.must_not_include:
            assert string not in result.markdown
