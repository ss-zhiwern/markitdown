...
class TestPdfMemoryOptimization:
    ...
    def test_page_close_called_on_every_page(self):
        ...
        with patch(
            "markitdown_liberated.converters._pdf_converter.pdfplumber"
        ) as mock_pdfplumber:
            mock_pdfplumber.open.side_effect = _mock_pdfplumber_open(pages)

            md = MarkItDown()
            buf = io.BytesIO(b"fake pdf content")
            from markitdown_liberated import StreamInfo
    ...
    def test_plain_text_pdf_falls_back_to_pdfminer(self):
        ...
        with patch(
            ...
            "markitdown_liberated.converters._pdf_converter.pdfminer"
        ) as mock_pdfminer:
            mock_pdfplumber.open.side_effect = _mock_pdfplumber_open(pages)
