...
import subprocess
from markitdown_liberated import __version__
...
def test_version() -> None:
    result = subprocess.run(
        ["python", "-m", "markitdown_liberated", "--version"], capture_output=True, text=True
    )
...
def test_invalid_flag() -> None:
    result = subprocess.run(
        ["python", "-m", "markitdown_liberated", "--foobar"], capture_output=True, text=True
    )
